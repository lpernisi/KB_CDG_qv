...
def api_riconciliazione_drill():
    ...
    if k == "fatt_nonsped":
        ...
        rows = righe("""SELECT TOP 300 sd.DocNo AS documento, ISNULL(cs.CompanyName, sd.CustSupp) AS cliente,
            ...
            FROM core.fatto_riga fr
            JOIN KODICEBAGNO_4.dbo.MA_SaleDoc sd ON sd.SaleDocId=fr.sale_doc_id
...
def api_ce():
    ...
    rows = righe(f"""
        SELECT {col} AS dim,
               ...
               SUM(f.mdc1 - f.mdc2)           AS commerciali,
...
async function caricaCE(){
  ...
  <p class="muted">Periodo: `
    ...
    + `. <em>Materiale</em> = nostro costo certificato (ricalcolo WAP); <em>Commerciali</em> = commissioni marketplace + piattaforma + provvigioni agenti; <em>Trasporto</em> = costo spedizione (fattura vettore o stima); <em>Margine</em> = dopo materiale, commerciali e trasporto. Imballi/Finanziari in costruzione.</p>`;
  h+=`<div class="panel" style="padding:0"><div style="max-height:62vh;overflow:auto"><table class="sticky"><thead><tr>
  ...
  h+=d.righe.map(r=>`<tr><td>${CEDIM!=='cliente'
    ...
    <td class="num">${eur(Number(r.fatturato)-Number(r.materiale)-Number(r.commerciali)-Number(r.trasporto))}</td><td class="num">${r.fatturato?(100*(Number(r.fatturato)-Number(r.materiale)-Number(r.commerciali)-Number(r.trasporto))/Number(r.fatturato)).toFixed(1):'0'}%</td>
...
async function ceDrill(el){
  ...
  inner+=`<table style="margin-top:6px"><thead><tr><th>Cliente</th><th>Codice</th><th>Categoria</th><th>Notes (Raggr.)</th>
     <th class="num">Fatturato</th><th class="num">Materiale</th><th class="num">% Mat</th><th class="num">Commerciali</th><th class="num">Trasporto</th><th class="num">Margine</th><th class="num">Nr. Ord.</th></tr></thead><tbody>`;
  inner+=R.map(x=>`<tr><td>${esc(x.cliente||'')}</td><td><code>${esc(x.codice||'')}</code></td>
     ...
     <td class="num">${eur(x.commerciali)}</td><td class="num">${eur(x.trasporto)}</td>
     <td class="num">${eur(Number(x.fatturato)-Number(x.materiale)-Number(x.commerciali)-Number(x.trasporto))}</td><td class="num">${num(x.n_ordini)}</td></tr>`).join("")
     || `<tr><td colspan="11" class="muted">Nessun cliente.</td></tr>`;
